...
import { getFieldSchedule, setMatchResult } from "@/lib/api/adminSchedule";
...
export default function AdminFieldDetailPage() {
  ...
  if (error || !field) return <ErrorState message={error ?? "Field not found."} onRetry={refresh} />;
  ...
  return (
    <div>
      ...
      <section className="pitch-hero-bg mt-4 rounded-[8px] p-6 text-white">
        ...
        <p className="mt-2 text-sm text-white/74">{field.complex?.name ?? "Complex"}</p>
      </section>
      ...
      <Card className="mt-6">
        <CardContent>
          <h2 className="mb-4 text-lg font-semibold text-neutral-950">Daily schedule</h2>
          <FieldSchedulePanel
            fieldId={field.id}
            ...
            fetchSchedule={getFieldSchedule}
            saveResult={async (matchId, h, a, note) => { await setMatchResult(matchId, h, a, note); }}
          />
        </CardContent>
      </Card>
    </div>
